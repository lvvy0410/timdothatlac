    ...
    <div class="container-fluid tm-mt-10" style="margin-left: 600px;">
        ...
        <div class="row tm-mb-50">
            <div class="col-lg-4 col-12 mb-5">
                <h2 class="tm-text-primary mb-4">Chỉnh sửa thông tin cá nhân</h2>
                <form action="{{ route('user-xl-chinh-sua-ho-so') }}" method="POST" class="tm-contact-form mx-auto">
                    @csrf
                    <div class="form-group">
                        <label class="tm-text-primary">Họ và tên</label>
                        <input type="text" name="ten" class="form-control rounded-0" value='{{Auth::user()->ten}}' />
                    </div>
                    <div class="form-group">
                        <label class="tm-text-primary">Số điện thoại</label>
                        <input type="text" name="so_dien_thoai" class="form-control rounded-0" value='{{Auth::user()->so_dien_thoai}}' />
                    </div>
                    <div class="form-group">
                        <label class="tm-text-primary">Email</label>
                        <input type="email" name="email" class="form-control rounded-0" value='{{Auth::user()->email}}' />
                    </div>
                    <div class="form-group">
                        <label class="tm-text-primary">Địa chỉ</label>
                        <textarea rows="5" name="dia_chi" class="form-control rounded-0">{{Auth::user()->dia_chi}}</textarea>
                    </div>
